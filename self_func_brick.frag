...
vec2 fract_func2(vec2 inp)
{
   vec2 outp = vec2(inp[0]-floor(inp[0]), inp[1]-floor(inp[1]));
   return outp;
}

float fract_func1(float inp)
{
...
void main()
{
   ...
   //  Stagger rows
   if (fract_func1(0.5*position.y)>0.5) position.x += 0.5;
   //  Scroll bricks at 2.5 bricks per second
   position.x += 2.5*time;
   //  Don't care about the integer part
   position = fract_func2(position);
   //  Snap to 0 or 1
